...
export type CaseAccess = "available" | "locked" | "redeem" | "completed";
...
export function CaseFolder({
  ...
  const isLocked = access === "locked" || access === "redeem";
  const requiresCard = access === "redeem";
    ...
    <div className="group relative pt-4">
      ...
      <article className={`relative border-[3px] border-border bg-surface shadow-[6px_6px_0_0_var(--color-warn),10px_10px_0_0_color-mix(in_oklab,var(--color-accent)_30%,transparent)] transition-[transform,box-shadow] duration-150 ${isLocked ? "opacity-70" : "group-hover:-translate-x-0.5 group-hover:-translate-y-0.5"}`}>
        <div className="flex items-center gap-2 border-b-[3px] border-border bg-surface-2 px-4 py-2">
          ...
          <Chip tone={isCompleted ? "green" : LEVEL_TONE[caseData.level]} className="ml-auto">{isCompleted ? t("case.completed") : requiresCard ? t("case.eventCard") : isLocked ? t("case.locked") : caseData.level}</Chip>
        </div>
        ...
        <div className="px-4 pb-4">
          ...
          {requiresCard ? (
            <ButtonLink href={localizePath("/redeem")} tone="accent" className="mt-4 w-full sm:w-auto">
              {t("case.redeemToUnlock")}
            </ButtonLink>
          ) : isLocked ? (
