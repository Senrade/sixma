...
export function CaseFolder({
  ...
  return (
    <div className="group relative pt-4">
      ...
      <article className={`relative rounded-[8px] border-2 border-ink bg-surface shadow-[6px_6px_0_0_var(--color-ink)] transition-[transform,box-shadow] duration-150 ${isLocked ? "opacity-70" : "group-hover:-translate-x-0.5 group-hover:-translate-y-0.5 group-hover:shadow-[8px_8px_0_0_var(--color-ink)]"}`}>
        <div className="flex items-center gap-2 border-b-2 border-ink bg-surface-2 px-4 py-2">
          ...
          <Chip tone={isCompleted ? "green" : LEVEL_TONE[caseData.level]} className="ml-auto">{isCompleted ? "Completed" : isLocked ? "Locked" : caseData.level}</Chip>
        </div>
          ...
          <div className="relative h-44 overflow-hidden rounded-[6px] border-2 border-ink bg-background">
            {revealDetails ? (
              ...
              <div className="case-grid-bg grid h-full place-items-center p-5 text-center">
                <div>
                  <span className="inline-grid h-11 w-11 place-items-center rounded-[6px] border-2 border-ink bg-ink font-mono text-xl font-black text-background">?</span>
                  <p className="mt-3 font-mono text-xs font-black uppercase">Evidence sealed</p>
                  <p className="mt-1 text-xs text-ink-soft">The dossier preview is revealed after completion.</p>
                </div>
              </div>
            )}
          </div>
          <span className="absolute right-3 top-4 -rotate-6 border-2 border-danger bg-background px-1.5 py-0.5 font-mono text-[10px] font-black uppercase text-danger" aria-hidden>{revealDetails ? "Evidence" : "Classified"}</span>
        </div>
        <div className="px-4 pb-4">
          ...
          {revealDetails ? <p className="mt-1.5 line-clamp-2 text-sm text-ink-soft">{caseData.short_summary}</p> : <p className="mt-1.5 text-sm text-ink-soft">Complete the full investigation to reveal this case dossier.</p>}
          ...
          {isLocked ? (
            <div className="mt-4 rounded-[6px] border-2 border-ink bg-muted px-4 py-2 text-center text-sm font-bold text-ink-soft">
              Complete {prerequisiteCaseId ?? "the previous case"} to unlock
            </div>
          ) : (
            <ButtonLink href={`/cases/${caseData.case_id}`} tone={isCompleted ? "secondary" : "accent"} className="mt-4 w-full sm:w-auto">
              {isCompleted ? "Review case" : "Investigate"} <span aria-hidden>-&gt;</span>
            </ButtonLink>
          )}
        </div>
      </article>
